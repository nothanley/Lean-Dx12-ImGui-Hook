	...
	_impl(new reshade::d3d12::swapchain_impl(command_queue->_device, original))
{
	...
	reshade::create_effect_runtime(_impl, command_queue->_orig);
	on_init(false);
}
...
public:
	...
	{
		(void)direct3d_device;
		(void)direct3d_version;
	}
};
...
void DXGISwapChain::on_init([[maybe_unused]] bool resize)
{
	...
	const unique_direct3d_device_lock lock(_direct3d_device, _direct3d_version, static_cast<D3D12CommandQueue *>(_direct3d_command_queue)->_mutex);
...
{
	// Some games test presentation for timing and composition purposes
	...
	// Also synchronize access to the command queue while events are invoked and the immediate command list may be accessed
	const unique_direct3d_device_lock lock(_direct3d_device, _direct3d_version, static_cast<D3D12CommandQueue *>(_direct3d_command_queue)->_mutex);
...
void DXGISwapChain::on_finish_present(HRESULT hr)
{
	...
	if (hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET)
	{
		...
		{
			const HRESULT reason = static_cast<ID3D12Device *>(_direct3d_device)->GetDeviceRemovedReason();
